...
type Marketplace = 'olx' | 'vinted' | 'wallapop' | 'amazon' | 'fnac' | 'temu'

export function exportForMarketplace(product: Product, marketplace: Marketplace): Record<string, string | number> {
...
function buildDescription(product: Product): string {
  ...
  if (product.weight_kg) lines.push(`Peso: ${product.weight_kg} kg`)
...
function buildBulletPoints(product: Product): string[] {
  const points: string[] = []
  if (product.brand) points.push(`Marca: ${product.brand}`)
  if (product.category) points.push(`Categoria: ${product.category}`)
  if (product.weight_kg) points.push(`Peso: ${product.weight_kg} kg`)
  if (product.attributes) Object.entries(product.attributes).slice(0, 2).forEach(([k, v]) => points.push(`${k}: ${v}`))
  points.push('Produto novo em embalagem original')
  return points
}

function mapVintedCategory(category?: string): string {
  if (!category) return 'Outros'
  const c = category.toLowerCase()
  if (c.includes('roupa') || c.includes('cloth') || c.includes('wear')) return 'Roupa'
  if (c.includes('sapato') || c.includes('shoe') || c.includes('calçado')) return 'Calçado'
  if (c.includes('eletro') || c.includes('electr')) return 'Eletrónica'
  if (c.includes('casa') || c.includes('home')) return 'Casa'
  return 'Outros'
}
