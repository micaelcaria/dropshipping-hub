...
export async function syncFromSheet(): Promise<SyncResult> {
  ...
  try {
    ...
    if (!res.ok) throw new Error(`HTTP ${res.status} ao obter a folha`)
    ...
    if (products.length === 0) throw new Error('Folha vazia ou formato inesperado')

    // Diff against previous data
    const added: string[] = []
    const removed: string[] = []
    const stockChanges: SyncResult['stockChanges'] = []
    ...
    const keyOf = (p: (typeof products)[number]) => `${p.supplier_ref}|${p.attributes.EAN}|${p.name}`

    if (existsSync(OUTPUT_FILE)) {
      const prev: typeof products = JSON.parse(readFileSync(OUTPUT_FILE, 'utf8'))
      const prevMap = new Map(prev.map(p => [keyOf(p), p]))
      const newMap = new Map(products.map(p => [keyOf(p), p]))

      for (const p of products) {
        const old = prevMap.get(keyOf(p))
        if (!old) { added.push(`${p.supplier_ref} ${p.name}`); continue }
        if ((old as any).stock !== p.stock) {
          stockChanges.push({ ref: p.supplier_ref, name: p.name, before: (old as any).stock ?? '', after: p.stock })
        }
      ...
      for (const p of prev) {
        if (!newMap.has(keyOf(p))) removed.push(`${p.supplier_ref} ${p.name}`)
      }
    }

    writeFileSync(OUTPUT_FILE, JSON.stringify(products, null, 2))

    const result: SyncResult = { ok: true, total: products.length, added, removed, stockChanges, priceChanges, syncedAt }
    ...
    const result: SyncResult = {
      ...
      syncedAt, error: (e as Error).message,
    }
