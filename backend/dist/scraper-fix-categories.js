...
const products = JSON.parse(readFileSync('scraper-output.json', 'utf8'));
    ...
    try {
        ...
        const result = await page.evaluate(() => {
            ...
            document.querySelectorAll('main a[href^="/produtos/"]').forEach(a => {
                const m = a.href.match(/\/produtos\/(\d+)$/);
                if (m)
        ...
        if (result.catName && result.ids.length > 0) {
            ...
            result.ids.forEach(id => {
                if (!idToCategory.has(id))
                    idToCategory.set(id, result.catName);
            });
        }
        else {
        ...
        console.log(`  cat=${catId} → erro: ${e.message.slice(0, 50)}`);
    }
}
await browser.close();
