...
export class VertenteHubScraper extends BaseScraper {
    ...
    async scrapeProducts(_maxPages) {
    ...
    async extractUrlsFromPage(page, url, urls) {
        try {
            await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
            // Wait for product cards to appear
            await page.waitForSelector('main a[href^="/produtos/"]', { timeout: 10000 }).catch(() => { });
            const links = await page.$$eval('main a[href^="/produtos/"]', els => els.map(a => a.href).filter(h => /\/produtos\/\d+$/.test(h)));
        ...
        const data = await page.evaluate(() => {
            ...
            if (mlPrice) {
                ...
                mlPrice.childNodes.forEach(node => {
                    if (node.nodeType === 3)
                        integerText += node.textContent?.trim(); // text nodes only
                });
            ...
            const imgs = [...new Set([...main.querySelectorAll('img')]
                    .map(i => i.src)
